...
"http://www.transfermarkt.co.uk/france/leistungsdaten/verein/3377/plus/0?reldata=FS%262015",
...
"http://www.transfermarkt.co.uk/norway/leistungsdaten/verein/3440/plus/0?reldata=POEM%262015"
]

national = [
"Italy","Slovakia","Russia","Denmark","Bosnia Herzegovina",
"Albania","Scotland","Poland","Sweden","Israel",
"Ireland","Serbia","Finland","Bulgaria","Feloe Islands",
"Estonia","Latvia","Azerbaijan","Cyprus","Moldova",
"Kazakhstan","Luxembourg","Montenegro","Liechtenstein","Greece",
"Georgia","France","Austria","Switzerland","Ukraine",
"Slovenia","Belarus","Romania","Armenia","Macedonia",
"Lithuania","Germany","Belgium","Portugal","Spain",
"Wales","England","Netherlands","Czech Republic","Croatia",
"Hungary","Northern Ireland","Turkey","Iceland","Norway"
]

connection = Mysql::new("127.0.0.1", "root", "motokokusanagi", "soccer_player")

for j in 49..(url.size-1)
  k=0
  p national[j]
  ...
  for i in 0..(team_url.size-1)
    st.execute name[i],age[i],height[i],position[i],national[j],"Europe",team[i],team_national[i],division[i],minutes[i],"Europe",team[i],team_national[i],division[i]
  end
