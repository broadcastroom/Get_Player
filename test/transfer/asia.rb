...
for j in 0..(url.size-1)
  ...
  for i in 0..(team_url.size-1)
    st.execute name[i],age[i],height[i],position[i],national[j],"Asia",team[i],team_national[i],division[i],minutes[i],"Africa",team[i],team_national[i],division[i],minutes[i]
  end
end
